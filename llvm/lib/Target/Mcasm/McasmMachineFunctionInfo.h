//===-- McasmMachineFunctionInfo.h - Mcasm machine function info ----*- C++ -*-===//
//
...
//===----------------------------------------------------------------------===//
//
// This file declares Mcasm-specific per-machine-function information.
//
//===----------------------------------------------------------------------===//

#ifndef LLVM_LIB_TARGET_Mcasm_McasmMACHINEFUNCTIONINFO_H
#define LLVM_LIB_TARGET_Mcasm_McasmMACHINEFUNCTIONINFO_H
...
class McasmMachineFunctionInfo;
...
struct McasmMachineFunctionInfo final : public yaml::MachineFunctionInfo {
  AMXProgModelEnum AMXProgModel;

  McasmMachineFunctionInfo() = default;
  McasmMachineFunctionInfo(const llvm::McasmMachineFunctionInfo &MFI);

  void mappingImpl(yaml::IO &YamlIO) override;
  ~McasmMachineFunctionInfo() override = default;
};

template <> struct MappingTraits<McasmMachineFunctionInfo> {
  static void mapping(IO &YamlIO, McasmMachineFunctionInfo &MFI) {
    YamlIO.mapOptional("amxProgModel", MFI.AMXProgModel);
  }
};
} // end namespace yaml

/// McasmMachineFunctionInfo - This class is derived from MachineFunction and
/// contains private Mcasm target-specific information for each MachineFunction.
class McasmMachineFunctionInfo : public MachineFunctionInfo {
  virtual void anchor();
  ...
  /// VarArgsFPOffset - Mcasm-64 vararg func fp reg offset.
...
public:
  McasmMachineFunctionInfo() = default;
  McasmMachineFunctionInfo(const Function &F, const TargetSubtargetInfo *STI) {}

  McasmMachineFunctionInfo(const McasmMachineFunctionInfo &) = default;
  ...
  void initializeBaseYamlFields(const yaml::McasmMachineFunctionInfo &YamlMFI);
