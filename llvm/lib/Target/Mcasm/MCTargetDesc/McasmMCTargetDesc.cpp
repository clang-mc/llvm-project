...
MCSubtargetInfo *llvm::Mcasm_MC::createMcasmMCSubtargetInfo(
  ...
  MCASM_DEBUG_LOG("DEBUG: createMcasmMCSubtargetInfo called\n");
  MCASM_DEBUG_LOG("DEBUG:   Triple = %s\n", TT.str().c_str());
  MCASM_DEBUG_LOG("DEBUG:   Triple.getArch() = %s\n", TT.getArchName().str().c_str());

  std::string ArchFS = Mcasm_MC::ParseMcasmTriple(TT);
  MCASM_DEBUG_LOG("DEBUG:   ArchFS = %s\n", ArchFS.c_str());
  ...
  MCASM_DEBUG_LOG("DEBUG:   About to call createMcasmMCSubtargetInfoImpl\n");

  MCSubtargetInfo *STI = createMcasmMCSubtargetInfoImpl(TT, CPUName, TuneCPU, ArchFS);

  MCASM_DEBUG_LOG("DEBUG: createMcasmMCSubtargetInfo completed\n");

  return STI;
}

static MCInstrInfo *createMcasmMCInstrInfo() {
  MCASM_DEBUG_LOG("DEBUG: createMcasmMCInstrInfo called\n");
  MCInstrInfo *X = new MCInstrInfo();
  MCASM_DEBUG_LOG("DEBUG: About to call InitMcasmMCInstrInfo\n");
  InitMcasmMCInstrInfo(X);
  MCASM_DEBUG_LOG("DEBUG: InitMcasmMCInstrInfo completed\n");
  return X;
}

static MCRegisterInfo *createMcasmMCRegisterInfo(const Triple &TT) {
  MCASM_DEBUG_LOG("DEBUG: createMcasmMCRegisterInfo called\n");
  // mcasm uses rax as the return address register (placeholder)
  (void)TT;
  unsigned RA = Mcasm::rax;

  MCRegisterInfo *X = new MCRegisterInfo();
  MCASM_DEBUG_LOG("DEBUG: About to call InitMcasmMCRegisterInfo\n");
  InitMcasmMCRegisterInfo(X, RA, 0, 0, RA);
  MCASM_DEBUG_LOG("DEBUG: InitMcasmMCRegisterInfo completed\n");
  return X;
}
...
static MCAsmInfo *createMcasmMCAsmInfo(const MCRegisterInfo &MRI,
  ...
  MCASM_DEBUG_LOG("DEBUG: createMcasmMCAsmInfo called\n");
  ...
  MCASM_DEBUG_LOG("DEBUG: MAI created, initializing frame state\n");
  ...
  MCASM_DEBUG_LOG("DEBUG: createMcasmMCAsmInfo completed\n");
  return MAI;
}
...
extern "C" LLVM_C_ABI void LLVMInitializeMcasmTargetMC() {
  ...
  MCASM_DEBUG_LOG("DEBUG: Registered ObjectTargetStreamer\n");

  // Register the asm target streamer.
  TargetRegistry::RegisterAsmTargetStreamer(*T, createMcasmAsmTargetStreamer);
  MCASM_DEBUG_LOG("DEBUG: Registered AsmTargetStreamer\n");
  ...
  MCASM_DEBUG_LOG("DEBUG: Registered NullTargetStreamer\n");

  // Register the MCInstPrinter.
  TargetRegistry::RegisterMCInstPrinter(*T, createMcasmMCInstPrinter);
  MCASM_DEBUG_LOG("DEBUG: Registered MCInstPrinter\n");

  // Register the asm backend
  TargetRegistry::RegisterMCAsmBackend(*T, createMcasm_32AsmBackend);
  MCASM_DEBUG_LOG("DEBUG: Registered MCAsmBackend\n");

  MCASM_DEBUG_LOG("DEBUG: LLVMInitializeMcasmTargetMC completed\n");
}
